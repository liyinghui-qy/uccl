...
void nv_gpu_reduce(void* sendbuff, void* recvbuff, int count, CCLDatatype datatype, CCLOp op, int root, Communicator* communicator, Stream* stream) {
    ...
    if (cudaStream == nullptr) {
        NCCLCHECK(ncclReduce(sendbuff, recvbuff, count, datatype_cuda, op_cuda, root, comm, 0));
    }
    else {
        NCCLCHECK(ncclReduce(sendbuff, recvbuff, count, datatype_cuda, op_cuda, root, comm, *cudaStream));
    }
}