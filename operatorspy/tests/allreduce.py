...
def test_cpu(lib):
    ...
    lib.Allreduce(descriptor, ctypes.byref(send_data), ctypes.byref(recv_data), data_size, datatypes.CCL_INT, ops.CCL_SUM, comm, None)
...
def test_gpu(lib):
    ...
    recv_data_host = torch.empty(data_size, dtype=torch.float32)
    ...
    for i, device_id in enumerate(devices_data):
        ...
        recv_data_gpu[i] = torch.empty(data_size, dtype=torch.float32).cuda()
...
if __name__ == "__main__":
    ...
    lib.Allreduce.restype = c_void_p
    lib.Allreduce.argtypes = [c_void_p, c_void_p, c_void_p, c_int, c_int, c_int, ctypes.POINTER(Communicator), c_void_p]
